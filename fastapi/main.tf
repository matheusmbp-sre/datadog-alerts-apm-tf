...
resource "datadog_monitor" "error_percentage" {
  ...
  query = "avg(last_30m):(sum:trace.aiohttp.request.errors{service:${var.service_name}, env:${var.environment}}.as_rate() * 100) / sum:trace.aiohttp.request.hits{service:${var.service_name}, env:${var.environment}}.as_rate() > ${var.error_critical_threshold}"
...
resource "datadog_monitor" "latency" {
  ...
  query = "avg(last_12h):anomalies(p75:trace.aiohttp.request{service:${var.service_name}, env:${var.environment}}.as_count(), 'agile', 2, direction='both', interval=120, alert_window='last_30m', count_default_zero='true', seasonality='hourly') > 0.75"
...
resource "datadog_monitor" "request" {  
  ...
  query = "avg(last_30m):sum:trace.aiohttp.request.hits{service:${var.service_name}, env:${var.environment}}.as_rate() > ${var.request_critical_threshold}"
